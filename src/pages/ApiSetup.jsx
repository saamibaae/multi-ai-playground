...
export default function ApiSetup() {
    ...
    const [local, setLocal] = useState({ ...keys, customBase: keys.customBase || '' })
    const [showSuccess, setShowSuccess] = useState(false)

    // Update local state when keys are loaded from Firebase
    useEffect(() => {
        setLocal({ ...keys, customBase: keys.customBase || '' })
    }, [keys])
    ...
    function handleClearKeys() {
        if (confirm('Are you sure you want to clear all API keys? This action cannot be undone.')) {
            clearKeys()
            setLocal({ openai: '', gemini: '', anthropic: '', zhipu: '', customBase: '' })
            setShowSuccess(false)
        }
    }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-neutral-100">
            <div className="card max-w-xl w-full">
                ...
                <div className="mt-4">
                    <label className="block text-sm mb-2">{active === 'custom' ? 'Base URL' : `${tabs.find(t => t.id === active)?.label} API Key`}</label>
                    <input
                        className="w-full rounded-lg border px-3 py-2"
                        placeholder={tabs.find((t) => t.id === active)?.placeholder}
                        type={active === 'custom' ? 'text' : 'password'}
                        value={active === 'custom' ? local.customBase || '' : local[active] || ''}
                        onChange={(e) =>
                            setLocal(
                                active === 'custom'
                                    ? { ...local, customBase: e.target.value }
                                    : { ...local, [active]: e.target.value },
                            )
                        }
                    />
                </div>
                ...
                <div className="mt-6 flex justify-between">
                    ...
                    <button
                        ...
                        disabled={!local.openai && !local.gemini && !local.anthropic && !local.zhipu && !local.customBase}
